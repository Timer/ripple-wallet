...
const waitForBalancesUpdate = (
  ...
  Promise.all([
    ...
    api
      .getBalances(destinationAddress, { currency: config.currency })
...
const questions = [
  {
    ...
    validate: value =>
      isNaN(parseInt(value)) ? 'Please enter a number' : true,
  },
  {
    type: 'input',
    ...
    validate: value =>
      RippleAddressCodec.isValidAddress(value)
        ? true
  ...
  },
  {
    ...
    validate: value =>
      value && isNaN(parseInt(value)) ? 'Please enter a number' : true,
  ...
  api
    ...
    .then(() => {
      ...
      return Promise.all([
        api.getBalances(sourceAddress, { currency: config.currency }),
        api
          .getBalances(answers.destinationAddress, {
        ...
        console.log(
          ...
          chalk.green(destinationBalance, config.currency)
        )
        if (destinationBalance + amount < config.baseReserve) {
          fail(
            `Send at least ${config.baseReserve} ${
              config.currency
            } to create the destination address`
          )
        }
        console.log(
          'Current sender balance:',
          chalk.green(sourceBalance, config.currency)
        )
        if (sourceBalance - amount < config.baseReserve) {
          fail(
            `There should be at least ${config.baseReserve} ${
              config.currency
            } remaining at the sender address`
          )
        }
        ...
        inquirer
          ...
          .then(confirm => {
            ...
            return api
              .preparePayment(sourceAddress, payment, instructions)
              .then(prepared => {
                const { signedTransaction } = api.sign(
                  prepared.txJSON,
                  answers.sourceSecret
                )
                ...
                return api.submit(signedTransaction).then(() => {
                  console.log(
                    'Waiting for balance to update (use Ctrl-C to abort)'
                  )
